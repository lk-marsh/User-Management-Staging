...
class TestToken(unittest.TestCase):
    ...
    def test_initiate_activate_user(self):
        result = mock_initiate_activate_user(
            url, token, mock_emails, mock_names)
        self.assertEqual(len(result), 2)

    def test_initiate_reset_password(self):
        result = mock_initiate_reset_password(
            url, token, mock_emails, mock_names)
        self.assertEqual(len(result), 2)

    def test_activate_or_reset(self):
        result = mock_activate_or_reset(
            url, token, mock_emails, mock_names, mock_user_tokens, mock_new_passwords)
        self.assertEqual(len(result), 2)
    ...
    def test_write_file(self):
        ...
        # Header of CSV File
        self.assertEqual(
            calls_list[0], unittest.mock.call('inactive emails\r\n'))

        # Contents of CSV
        self.assertEqual(calls_list[1], unittest.mock.call(
            'inactive email 1\r\n'))
        self.assertEqual(calls_list[2], unittest.mock.call(
            'inactive email 2\r\n'))
